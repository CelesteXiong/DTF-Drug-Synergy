...
data = read.csv('C:/Users/xiongxiaoji/Desktop/41467_2018_4647_MOESM4_ESM.csv')
# data = data[1:100]
...
{
  all_info$gene2 = gsub(pattern 
                              = all_gene[i], i,all_info$gene2)
  all_info$gene1 = gsub(pattern 
                              = all_gene[i], i,all_info$gene1)
}
...
for(p in 1: all_cancer_type_length){
  ...
  tmp_o = matrix(nrow = all_gene_length+1, ncol = all_gene_length+1)
  test_num = 37
  ans = 0
