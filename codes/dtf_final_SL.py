...
from utils.visualize import plot_metric
...
model_name = 'DTF-SL'
result_root = r'C:\Users\xiongxiaoji\Documents\GitHub\DTF-Drug-Synergy\result'
early_stopping_cb = keras.callbacks.EarlyStopping(patience = 3, restore_best_weights = True)
checkpointer = ModelCheckpoint(monitor='val_acc', filepath=os.path.join(result_root + '\\model-{epoch:02d}.hdf5'), verbose=1, save_best_only=True)

history = model.fit(X_train, y_train, epochs = 10 ,batch_size = 128,
             validation_data = (X_val,y_val), callbacks = [early_stopping_cb, checkpointer])

plot_metric(config['result_root'], model_name, history.history['acc'], history.history['val_acc'], 'accuracy')
